...
class QuickchatTextBox extends React.Component {
  constructor() {
    super();
    this.state = {
      user: {},
  ...
  findEnter() {
    $('#quickchatMessageInput').keypress((event) => {
      if (event.which === 13) {
  ...
  handleClick(id, attachment) {
    if (this.state.commentBody) {
      ...
      const elem = document.getElementById('quickchatMessageInput');
      elem.value = '';
    }
  }
  ...
  upload() {
    const myFile = $('#fileInputQuickchat').prop('files');
    this.setState({ file: myFile[0]});
  }

  render() {
    return(
        <div className="quickchatTextBox">
            ...
            {this.state.emojiIsOpen ?
                ...
                : null}
            <div className="quickchatInputWrapper">
                <Form className="quickchatTextBoxForm">
                      <TextArea
                          id="quickchatMessageInput"
                          autoHeight
                ...
                <div className="actionsTextBox">
                    <Icon id="fileUploadQuickChat" onClick={() => $('#fileInputQuickchat').trigger('click')} className="attachFileIconModal" name="attach" size="large"/>
                    <input id="fileInputQuickchat" type="file" onChange={() => this.upload()} />
...
const mapStateToProps = (state) => ({
  ...
  postData: state.quickchatReducer.postData
});

const mapDispatchToProps = (dispatch) => ({
});

export default connect(mapStateToProps, mapDispatchToProps)(QuickchatTextBox);
