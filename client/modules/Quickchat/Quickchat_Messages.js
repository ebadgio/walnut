...
class QuickchatMessages extends React.Component {
  constructor() {
    super();
    this.state = {
      user: {},
  ...
  componentDidMount() {
    console.log('will', this.props.postData);
  ...
  startListen(data, user) {
    ...
    if (data.postId) {
      console.log('inside message function', data.postId);
      const messagesRef = firebaseApp.database().ref('/messages/' + data.postId).orderByKey().limitToLast(20);
      messagesRef.on('value', (snapshot) => {
        console.log('snapshot', snapshot.val(), messagesRef);
        if (snapshot.val()) {
          const send = _.values(snapshot.val());
          console.log('inside inside function', send);
  ...
  render() {
    ...
    return(
        <div className="quickchatMessages">
            <InfiniteScroll
            ...
            <div className="quickchatTypingRow">
...
export default connect(mapStateToProps, mapDispatchToProps)(QuickchatMessages);
